 ...
 *
 *    Description:  ClipboardImpl class declaration 
 *
 *        Version:  1.0
 *        Created:  29/01/22 15:36:15
 *       Revision:  none
 *       Compiler:  gcc
 *
...
// Headers
////////////////////////////////////////////////////////////
#include "../unix/ClipboardImpl.hpp"
#include "../unix/Display.hpp"
#include "../unix/Clock.hpp"
...
m_requestResponded(false)
{
    ...
    XSelectInput(m_display, m_window, SelectionNotify | 
...
String ClipboardImpl::getStringImpl()
{
    ...
    while (!m_requestResponded && (clock.getElapsedTime().asMilliseconds() < 
                1000))
        processEvents();

    // If no response was received within the time period, 
    // clear our clipboard contents
...
void ClipboardImpl::setStringImpl(const String& text)
{
    ...
    // Check if setting the selection owner was succesxwcul
    if (XGetSelectionOwner(m_display, m_clipboard) != m_window)
        err() << "Cannot set clipboard string: Unable to get ownership \
            of X selection" << std::endl;
}
...
void ClipboardImpl::processEventsImpl()
{
    ...
    while (XCheckIfEvent(m_display, &event, &checkEvent, 
...
void ClipboardImpl::processEvent(XEvent& windowEvent)
{
    switch (windowEvent.type)
    {
        ...
        case SelectionNotify:
        {
            ...
            // If retrieving the selection fails or conversion is unsuccesxwcul
            ...
            if ((selectionEvent.property == None) || 
                    (selectionEvent.selection != m_clipboard))
                break;
            ...
            if (result == Success)
            {
                ...
                // The selection requestor must always delete the 
                // property themselves
        ...
        case SelectionRequest:
        {
            ...
            if (selectionRequestEvent.selection == m_clipboard)
            {
                if (selectionRequestEvent.target == m_targets)
                {
                    ...
                    // Notify the requestor that they can read the targets 
                    // from their window property
                    selectionEvent.target = m_targets;

                    XSendEvent(m_display, selectionRequestEvent.requestor, 
                            True, NoEventMask, 
                         ...
                         (selectionRequestEvent.target == m_text)))
                {
                    ...
                    // Notify the requestor that they can read the 
                    // data from their window property
                    ...
                    break;
                        ...
                        ((selectionRequestEvent.target == m_utf8String) || 
                         (selectionRequestEvent.target == m_text)))
                {
                    ...
                    // Notify the requestor that they can read the data 
                    // from their window property

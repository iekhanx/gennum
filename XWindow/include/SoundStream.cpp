 ...
 * ========================================================================
 *
 *       Filename:  SoundStream.cpp
 *
 *    Description:  SoundStream class implementation
 *
 *        Version:  1.0
 *        Created:  26/01/22 12:05:29
 *       Revision:  none
 *       Compiler:  gcc
 *
...
void SoundStream::initialize(unsigned int channelCount, unsigned int sampleRate)
{
    ...
    if (m_format == 0)
    {
        m_channelCount = 0;
        m_sampleRate   = 0;
        err() << "Unsupported number of channels (" << m_channelCount << ")" 
              << std::endl;
    }
}
...
void SoundStream::play()
{
    ...
    if (m_format == 0)
    {
        err() << "Failed to play audio stream: sound parameters have not \ 
            been initialized (call initialize() first)" << std::endl;
        return;
    }
    ...
    // Start updating the stream in a separate thread to avoid 
...
void SoundStream::setPlayingOffset(Time timeOffset)
{
    ...
    // Restart streaming
    m_samplesProcessed = static_cast<Uint64>(timeOffset.asSeconds() * 
                        static_cast<float>(m_sampleRate)) * m_channelCount;
...
Time SoundStream::getPlayingOffset() const
{
    if (m_sampleRate && m_channelCount)
    {
        ...
        return seconds(secs + static_cast<float>(m_samplesProcessed) / 
...
void SoundStream::streamData()
{
    ...
    for (;;)
    {
        ...
        // Get the number of buffers that have been processed (i.e. 
        // ready for reuse)
        ALint nbProcessed = 0;
        ...
        while (nbProcessed--)
        {
            ...
            {
                // This was the last buffer before EOF or Loop End: 
                // reset the sample count
                m_samplesProcessed = static_cast<Uint64>
                                (m_bufferSeeks[bufferNum]);
            ...
            else
            {
                ...
                // Bits can be 0 if the format or parameters are 
                // corrupt, avoid division by zero
                if (bits == 0)
                {
                    err() << "Bits in sound stream are 0: make sure that \
                        the audio format is not corrupt "
                ...
                {
                    m_samplesProcessed += static_cast<Uint64>(size / 
                                            (bits / 8));
                }
            }
                                    ...
                                    bool immediateLoop)
{
    ...
    for (Uint32 retryCount = 0; !onGetData(data) && (retryCount < 
                BufferRetries); ++retryCount)
    {
        ...
        // using onLoop(), and store the result in the buffer seek array
        // This marks the buffer as the "last" one (so that we know where 
        // to reset the playing position)
        m_bufferSeeks[bufferNum] = onLoop();

        // If we got data, break and process it, else try to fill 
        // the buffer once again
        if (data.samples != nullptr && data.sampleCount != 0)
            break;

        // If immediateLoop is specified, we have to immediately 
        // adjust the sample count
        if (immediateLoop && (m_bufferSeeks[bufferNum] != NoLoop))
        {
            // We just tried to begin preloading at EOF or Loop End: 
            // reset the sample count
    ...
    if (data.samples && data.sampleCount)
    {
        ...
        alCheck(alBufferData(buffer, m_format, data.samples, size, 
...
bool SoundStream::fillQueue()
{
    ...
    {
        // Since no sound has been loaded yet, we can't schedule loop 
        // seeks preemptively,
        // So if we start on EOF or Loop End, we let fillAndPushBuffer() 
        // adjust the sample count

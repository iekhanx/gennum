 ...
 * ========================================================================
 *
 *       Filename:  JoystickImpl.cpp
 *
 *    Description:  JoystickImpl class implementation 
 *
 *        Version:  1.0
 *        Created:  29/01/22 01:46:12
 *       Revision:  none
 *       Compiler:  gcc
 *
...
namespace
{
    ...
    bool isJoystick(udev_device* udevDevice)
    {
        ...
        // On some platforms (older udev), ID_INPUT_ properties 
        // are not present, instead
        // the system makes use of the ID_CLASS property to 
        // identify the device class
        const char* idClass = udev_device_get_property_value(udevDevice, 
                "ID_CLASS");
    ...
    void updatePluggedList(udev_device* udevDevice = nullptr)
    {
        if (udevDevice)
        {
            ...
            if (action)
            {
                if (isJoystick(udevDevice))
                {
                    ...
                    for (recordIt = joystickList.begin(); 
                            recordIt != joystickList.end(); ++recordIt)
                    {
                        if (recordIt->deviceNode == devnode)
                        {
                            if (std::strstr(action, "add"))
                            {
                                // The system path might have changed 
                                // so update it
                                const char* syspath = 
                    ...
                    if (recordIt == joystickList.end())
                    {
                        ...
                        {
                            // Not mapped during the initial scan, 
                            // and removed (shouldn't happen)
                            xwc::err() << 
                                "Trying to disconnect joystick that wasn't \
                                connected" << std::endl;
                        }
                    }
                }
        ...
        // Reset the plugged status of each mapping since we are doing 
        // a full rescan
        ...
        if (!udevEnumerator)
        {
            xwc::err() << "Error while creating udev enumerator" << std::endl;
            return;
        }
        ...
        if (result < 0)
        {
            xwc::err() << "Error while adding udev enumerator match" 
                      << std::endl;
            return;
        }
        ...
        if (result < 0)
        {
            xwc::err() << "Error while enumerating udev devices" << std::endl;
            return;
        }

        udev_list_entry* devices = 
            udev_enumerate_get_list_entry(udevEnumerator);
        udev_list_entry* device;

        udev_list_entry_foreach(device, devices) {
            const char* syspath = udev_list_entry_get_name(device);
            udev_device* newUdevDevice = 
                udev_device_new_from_syspath(udevContext, syspath);
            ...
            if (newUdevDevice && isJoystick(newUdevDevice))
            {
                ...
                for (recordIt = joystickList.begin(); recordIt != 
                                        joystickList.end(); ++recordIt)
                {
                    if (recordIt->deviceNode == devnode)
                    {
                        recordIt->plugged = true;
    ...
    bool hasMonitorEvent()
    {
        ...
        return (select(monitorFd + 1, &descriptorSet, nullptr, nullptr, 
                    &timeout) > 0) &&
               FD_ISSET(monitorFd, &descriptorSet);
    }

    // Get a property value from a udev device
    const char* getUdevAttribute(udev_device* udevDevice, 
            const std::string& attributeName)
    {
        return udev_device_get_property_value(udevDevice, 
                attributeName.c_str());
    }

    // Get a system attribute from a USB device
    const char* getUsbAttribute(udev_device* udevDevice, 
            const std::string& attributeName)
    {
        udev_device* udevDeviceParent = 
            udev_device_get_parent_with_subsystem_devtype(udevDevice, 
                    "usb", "usb_device");

        if (!udevDeviceParent)
            return nullptr;

        return udev_device_get_sysattr_value(udevDeviceParent, 
                attributeName.c_str());
    }

    // Get a USB attribute for a joystick as an unsigned int
    unsigned int getUsbAttributeUint(udev_device* udevDevice, 
            const std::string& attributeName)
    {
        if (!udevDevice)
            return 0;
        ...
        if (attribute)
            value = static_cast<unsigned int>(std::strtoul(attribute, 
                        nullptr, 16));
    ...
    unsigned int getUdevAttributeUint(udev_device* udevDevice, 
            const std::string& attributeName)
    {
        if (!udevDevice)
            return 0;
    ...
    unsigned int getJoystickVendorId(unsigned int index)
    {
        if (!udevContext)
        {
            xwc::err() << "Failed to get vendor ID of joystick " 
                     << joystickList[index].deviceNode << std::endl;
            return 0;
        }

        udev_device* udevDevice = udev_device_new_from_syspath(udevContext, 
                joystickList[index].systemPath.c_str());
    ...
    unsigned int getJoystickProductId(unsigned int index)
    {
        if (!udevContext)
        {
            xwc::err() << "Failed to get product ID of joystick " 
                      << joystickList[index].deviceNode << std::endl;
            return 0;
        }
    ...
    std::string getJoystickName(unsigned int index)
    {
        ...
        xwc::err() << "Unable to get name for joystick " << devnode << std::endl;
...
void JoystickImpl::initialize()
{
    ...
    if (!udevContext)
    {
        xwc::err() << "Failed to create udev context, joystick support \
    ...
    if (!udevMonitor)
    {
        err() << "Failed to create udev monitor, joystick connections \
            and disconnections won't be notified" << std::endl;
    }
    else
    {
       int error = udev_monitor_filter_add_match_subsystem_devtype(udevMonitor,
                    "input", nullptr);

        if (error < 0)
        {
            err() << "Failed to add udev monitor filter, joystick connections \
                and disconnections won't be notified: " << error << std::endl;
        ...
        else
        {
            ...
            if (error < 0)
            {
                err() << "Failed to enable udev monitor, joystick connections \
                    and disconnections won't be notified: " 
                    << error << std::endl;
...
bool JoystickImpl::open(unsigned int index)
{
    ...
    if (joystickList[index].plugged)
    {
        ...
        {
            err() << "Failed to open joystick " << devnode << ": " 
                  << errno << std::endl;
        }
    }
            ...
            case JS_EVENT_AXIS:
            {
                ...
                if (joyState.number < ABS_MAX + 1)
                {
                    switch (m_mapping[joyState.number])
                    {
                        ...
                        case ABS_Z:
                        case ABS_THROTTLE: m_state.axes[Joystick::Z]= value;
                                           break;
                        case ABS_RZ:
